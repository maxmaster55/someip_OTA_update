...
bool DownloadClient::connect() {
    ...
    for (int i = 0; i < MAX_CONNECT_RETRIES; ++i) {
        ...
        std::cout << "[DownloadClient] Waiting for service... (attempt " << (i + 1) << "/" << MAX_CONNECT_RETRIES << ")" << std::endl;
        std::this_thread::sleep_for(2s);
    }

    std::cerr << "[DownloadClient] Failed to connect to Update service after " << MAX_CONNECT_RETRIES << " attempts" << std::endl;
    return false;
}

void DownloadClient::connectAsync() {
    runtime_ = CommonAPI::Runtime::get();
    if (!runtime_) {
        std::cerr << "[DownloadClient] Failed to get CommonAPI Runtime" << std::endl;
        return;
        ...
        proxy_->getProxyStatusEvent().subscribe(
            [this](const CommonAPI::AvailabilityStatus& status) {
                if (status == CommonAPI::AvailabilityStatus::AVAILABLE) {
                    std::cout << "[DownloadClient] Service now available" << std::endl;
                }
            }
        );
    }
...
bool DownloadClient::checkForUpdate(uint32_t& versionId, int64_t& fileSize,
    ...
    return callStatus == CommonAPI::CallStatus::SUCCESS;
}
